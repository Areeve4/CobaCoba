...
"");tmpBookConfig.isMouseEnterToDisplayArticleTitle=getConfigValue("isMouseEnterToDisplayArticleTitle",!1);tmpBookConfig.delayToFlipFirst=getConfigValue("delayToFlipFirst",!1);tmpBookConfig.timeToFlipFirst=getConfigValue("timeToFlipFirst",10);tmpBookConfig.printBackgroundPosition=getConfigValue("printBackgroundPosition","fit");try{if(window.ignoreModuleArray&&window.ignoreModuleArray.bookConfig)for(e=0;e<window.ignoreModuleArray.bookConfig.length;e++)f=window.ignoreModuleArray.bookConfig[e],void 0!=
tmpBookConfig[f]&&(tmpBookConfig[f]=!1)}catch(l){}checkFliphtml5PagesConfig();checkFlipStyleConfig();return"issuu"==tmpBookConfig.FlipStyle?k.yield(loadCSS(tmpBookConfig.UIBaseURL+"style/issuubook.min.css"),3):k.yield(loadCSS(tmpBookConfig.UIBaseURL+"style/flipbook.min.css"),3)}checkPageEditorConfig();recordError();g=Module._getVerifyString();h=UTF8ToString(g);Module._free(g);eval(h);k.jumpToEnd()})};
...
var VERSION_NAME="6.4.9",BUILD_DATE="2025060301",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8,documentView_book:9,singleDocumentView_book:10},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},singleBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},bookCorner={top_left:0,top_right:1,bottom_left:2,bottom_right:3},BrowserType={WEBKIT:1,
...
BookType.single_book;initBookFrame();break;case "catalog_book":bookType=BookType.catalog_book,initBookFrame()}},getBookTye:function(){var b="normal_book";if("slide"==bookConfig.FlipStyle.toLowerCase()||isBelowIE9())b="slide_book";"flip"!=bookConfig.FlipStyle.toLowerCase()||isBelowIE9()||(b="flip_book");"issuu"!=bookConfig.FlipStyle.toLowerCase()||isBelowIE9()||(b="flip_book");""!=bookConfig.FlipStyle||isBelowIE9()||(b="flip_book");"catalog"!=bookConfig.FlipStyle.toLowerCase()||isBelowIE9()||(b="catalog_book");
...
this.$parent.width(),c=this.$parent.width(),this.hideSmallTab||(b/=2),this.leftBar.css({width:b+"px",left:"0px"}),this.rightBar.css({width:b+"px",left:c-b+"px"}),rightToLeft?(this.resizeItemsOfBar(this.rightBar,"left",1),this.resizeItemsOfBar(this.leftBar,"left",-1)):(this.resizeItemsOfBar(this.leftBar,"left",1),this.resizeItemsOfBar(this.rightBar,"left",-1)),!isPad()&&!isPhone()&&!this.isRefresh&&this.bookmarkList.count()&&75>BookInfo.top()-templateArea.getTopHeight()&&(bookConfig.topMargin=75,this.isRefresh=
!0)},resizeItemsOfBar:function(b,c,d){for(var e=b.width(),f=0,g=0,h=0;h<this.bookmarkList.count();h++){var k=this.bookmarkList.item(h),l=this.bookmarkPool.getBookmarkItem(k);l.$parent==b&&(f=l.getWidth()+22,g+=l.getWidth()+22)}g+=2;h=b.children().length;Number.between(Math.floor((e-4-f)/(h-1)),10,65);var n=f=0,p=0;for(h=0;h<this.bookmarkList.count();h++)if(k=this.bookmarkList.item(h),l=this.bookmarkPool.getBookmarkItem(k),l.$parent==b){var t={};g>=e?(0<p&&(n=l.getWidth()>2*p/3?n+(p/2+10):n+(p-l.getWidth()/
2+10)),t[c]=n,p=l.getWidth()):(t[c]=f,f+=l.getWidth()+22);t["z-index"]=d*k.pageIndex;l.setPosition(t)}},show:function(){this.visible=!0;this.refresh()},hide:function(){this.hideLeftBar();this.hideRightBar();this.visible=!1},showLeftBar:function(){0<this.leftBar.children().length?this.leftBar.css("display","block"):this.leftBar.css("display","none")},hideLeftBar:function(){this.leftBar.css("display","none")},showRightBar:function(){0<this.rightBar.children().length?this.rightBar.css("display","block"):
this.rightBar.css("display","none")},hideRightBar:function(){this.rightBar.css("display","none")}});
...
1,!1,b);break;case BookType.singleCatalog_book:gotoPageFun(parseInt(singleCatalogBook.currentIndex)-1,!1,b);break;case BookType.documentView_book:gotoPageFun(BookInfo.getMinPageIndex(singleDocumentViewBook.getRealPageIndex())-2,!1,b);break;case BookType.singleDocumentView_book:gotoPageFun(parseInt(singleDocumentViewBook.getRealPageIndex())-1,!1,b)}},firstPageFun=function(b){gotoPageFun(1,!1,b)},lastPageFun=function(b){gotoPageFun(originTotalPageCount,!1,b)},delayToFlipFirstTimer=null,gotoPageFun=
...
BookInfo.getCurrentPages().indexOf(e)&&b>e){gotoPageFun(e,c,d);return}if(0>BookInfo.getCurrentPages().indexOf(1)&&1>b){gotoPageFun(1,c,d);return}if(bookConfig.isShowFirstOrLastPagePrompt){b=1>b?getLanguage("lblFirst","This is the first page"):getLanguage("lblLast","This is the last page");global._tipsBox||(_tipsBox=new TipsBox(b,bookContainer));_tipsBox.setCaption(b);_tipsBox.showTipsBox();return}}d=b;BookInfo.isDoublePage()&&(d=BookInfo.getMaxPageIndex(d));d=Math.max(1,d);if(passwardType()&&bookConfig.excludeFrontPages&&
d>=parseInt(bookConfig.excludeFrontPages)&&!verifyPassward()&&(global.passwardPannel||(global.passwardPannel=new PasswardPannel),global.passwardPannel.show(),d>parseInt(bookConfig.excludeFrontPages)))return;switch(bookType){case BookType.normal_book:flipBook.gotoPage(b,c);break;case BookType.single_book:singlePageBook.gotoPage(b,c);break;case BookType.slide_book:slideBook.gotoPage(b,c);break;case BookType.singleSlide_book:singleSlideBook.gotoPage(b,c);break;case BookType.catalog_book:catalogBook.gotoPage(b,
c);break;case BookType.singleCatalog_book:singleCatalogBook.gotoPage(b,c);break;case BookType.documentView_book:singleDocumentViewBook.gotoPage(b,c);break;case BookType.singleDocumentView_book:singleDocumentViewBook.gotoPage(b,c)}global.bookmark&&global.bookmark.elements.refreshElements();bookConfig.SelectTextButtonVisible&&BookInfo.getBook().unSelect();bookConfig.MagnifierButtonVisible&&BookInfo.getBook().unMagnifierZoom();global.annotationPannel&&global.annotationPannel.clearBtnDownCase();bookConfig.addCurrentPage&&
global.sharePanel&&global.sharePanel.elements&&global.sharePanel.elements.fillArea(!0,"link");global.downLoadBody&&downLoadBody.gotoPage(b);if(bookConfig.AnnotationButtonVisible&&!isPhone()&&!isPad()&&BookInfo.getBook().bookAttachments.noteAdd)BookInfo.getBook().bookAttachments.noteAdd.onResize();setCurrentIndexTextField(BookInfo.getCurrentPageIndex(),BookInfo.isDoublePage());setLocationHash(BookInfo.getCurrentPageIndex());global.phoneGotoPagePanel&&global.phoneGotoPagePanel.visible&&global.phoneGotoPagePanel.refreshSlider();
...
{x:n,y:p},t.rotateY?(t.$page[0].style.removeProperty("transform-origin"),d=void 0,B=transformCSS({scale:{x:n,y:p},perspective:6E3,rotateY:null!=(d=t.rotateY)?d:0})):(d=void 0,B=transformCSS({origin:g,scale:{x:n,y:p},perspective:6E3,rotateY:null!=(d=t.rotateY)?d:0})));t.setCSSByStyle(B);if(t.page.pageFlipShadow){var v=h?transformCSS({scale:{x:k.x/n,y:1},origin:g}):transformCSS({scale:{x:1,y:1},origin:g});t.page.pageFlipShadow&&t.page.pageFlipShadow.setCSSByStyle(v)}}}return{x:n,y:p}}catch(w){}},getTranScaleByIndex:function(b){return this.getFitWHByIndex(b)},
